...
class LogSystem:
    ...
    def retrieve(self, start: str, end: str, granularity: str):
        ...
        print(self.events)
        lind = bisect_left(self.events, (start_ts,0))
        rind = bisect_left(self.events, (end_ts,0))
        ...
        if rind < len(self.events) and self.events[rind] == end_ts:
            rind -= 1

        res = []
        contains = set()
        for event in self.events[lind:rind]:
            if event[1] not in contains:
                res.append(event[1])
            contains.add(event[1])
    ...
    def rewrite_ts(self, ts, granularity, upper:bool=True):
        ...
        if granularity == "Year":
            ...
            if dt.month == 12:
                nxt_dt = dt.replace(year=dt.year + 1).replace(month=1)
            else:
                nxt_dt = dt.replace(month=dt.month+1)
