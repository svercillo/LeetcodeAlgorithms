...
# name to bank is wrong
# too many banks is too large
# which bank ID is linked succesfully
def create_routing_number_mapping(
        rn_to_name_arr: List[Dict[str, str]], name_to_bank_id: List[Tuple[str, int]],
                                    per_routing_number_distributions: dict, baseline_distribution: dict
                                  ) -> Dict[str, List[int]]:
    ...
    for rn in res:
        mapping = res[rn]
        res[rn] = sorted([(mapping[k], k) for k in mapping], reverse=True)
